        ...
        for (int i=0; i<8; ++i)
            if(dyn->lsx.x87cache[i]!=-1) {
                ...
                switch(lsxcache_get_current_st(dyn, ninst, st)) {
                    MESSAGE(LOG_INFO, "=== x87 x87_purgecache, try store ST%d to  offset %d  stack %d\n",
                             st, (dyn->lsx.x87cache[i]&7) <<3, dyn->lsx.stack );
                    case LSX_CACHE_ST_D:
...
static void x87_reflectcache(dynarec_la64_t* dyn, int ninst, int s1, int s2, int s3)
{
    ...
    for (int i=0; i<8; ++i)
        if(dyn->lsx.x87cache[i]!=-1) {
            ...
            ALSL_D(s1, s2, xEmu, 2);
                                MESSAGE(LOG_INFO, "=== x87 x87_reflectcache, try store ST%d to  offset %d  stack %d\n",
                             i, (dyn->lsx.x87cache[i]&7) <<3, dyn->lsx.stack );
